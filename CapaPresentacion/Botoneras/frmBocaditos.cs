...
{
    public partial class frmBocaditos : Form, ISelectorProducto
    {
        ...
        public frmBocaditos()
        {
            ...
            this.Load += frmBocaditos_Load;

            flpBocaditos.VerticalScroll.Visible = false;
            ...
            flpBocaditos.MouseDown += flpBocaditos_MouseDown;
            flpBocaditos.MouseMove += flpBocaditos_MouseMove;
            flpBocaditos.MouseUp += flpBocaditos_MouseUp;
        }

        private void frmBocaditos_Load(object sender, EventArgs e)
        {
            ...
            flpBocaditos.AutoScroll = true; // NECESARIO para que calcule el área de scroll

            // (Opcional) suaviza el repintado
            typeof(Panel).GetProperty("DoubleBuffered",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
                ?.SetValue(flpBocaditos, true);

            // 🔹 Ocultar barras de scroll, pero mantener AutoScroll funcionando
            flpBocaditos.HorizontalScroll.Enabled = false;
            flpBocaditos.HorizontalScroll.Visible = false;
            flpBocaditos.VerticalScroll.Enabled = false;
            flpBocaditos.VerticalScroll.Visible = false;

            // (Si ya tienes botones en el diseñador)
            foreach (Control c in flpBocaditos.Controls)
                WireChild(c);

            flpBocaditos.ControlAdded += (_, ev) => WireChild(ev.Control);

            flpBocaditos.AutoScrollPosition = new Point(1, 1);
        }
        ...
        {
            // Drag en los hijos
            c.MouseDown -= flpBocaditos_MouseDown;
            c.MouseMove -= flpBocaditos_MouseMove;
            c.MouseUp -= flpBocaditos_MouseUp;
            c.MouseDown += flpBocaditos_MouseDown;
            c.MouseMove += flpBocaditos_MouseMove;
            c.MouseUp += flpBocaditos_MouseUp;
        ...
        private void Child_EnterRedirectFocus(object sender, EventArgs e) => flpBocaditos.Focus();
        private void Child_MouseDownRedirectFocus(object sender, MouseEventArgs e) => flpBocaditos.Focus();

        private void flpBocaditos_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            _originScrollX = -flpBocaditos.AutoScrollPosition.X;
            _lastX = _dragStartX;
            _lastDX = 0;
            _velocity = 0;
            _inertiaTimer?.Stop();
        ...
        private void flpBocaditos_MouseMove(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            ...
            int currentY = -flpBocaditos.AutoScrollPosition.Y;
            flpBocaditos.AutoScrollPosition = new Point(target, currentY);

            _lastDX = x - _lastX;
            _lastX = x;
        }

        private void flpBocaditos_MouseUp(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            _dragging = false;
        ...
        {
            int currentX = -flpBocaditos.AutoScrollPosition.X;
            int currentY = -flpBocaditos.AutoScrollPosition.Y;
        ...
        {
            int overflow = flpBocaditos.DisplayRectangle.Width - flpBocaditos.ClientSize.Width;
            return Math.Max(0, overflow);
        }

        private int MouseXEnFlp() => flpBocaditos.PointToClient(Cursor.Position).X;
