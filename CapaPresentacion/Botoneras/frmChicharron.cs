...
{
    public partial class frmChicharron : Form, ISelectorProducto
    {
        ...
        public frmChicharron()
        {
            InitializeComponent();
        }

        private void frmChicharron_Load(object sender, EventArgs e)
        {
            ...
            flpChicharron.AutoScroll = true; // NECESARIO para que calcule el área de scroll

            // (Opcional) suaviza el repintado
            typeof(Panel).GetProperty("DoubleBuffered",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
                ?.SetValue(flpChicharron, true);

            // 🔹 Ocultar barras de scroll, pero mantener AutoScroll funcionando
            flpChicharron.HorizontalScroll.Enabled = false;
            flpChicharron.HorizontalScroll.Visible = false;
            flpChicharron.VerticalScroll.Enabled = false;
            flpChicharron.VerticalScroll.Visible = false;

            // (Si ya tienes botones en el diseñador)
            foreach (Control c in flpChicharron.Controls)
            {
                if (c is Button b)
                {
                    b.MouseDown += flpChicharron_MouseDown;
                    b.MouseMove += flpChicharron_MouseMove;
                    b.MouseUp += flpChicharron_MouseUp;
                }
            }

            flpChicharron.AutoScrollPosition = new Point(1, 1);
        }
        ...
        private void flpChicharron_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            _originScrollX = -flpChicharron.AutoScrollPosition.X;
            _lastX = _dragStartX;
            _lastDX = 0;
            _velocity = 0;
            _inertiaTimer?.Stop();
        ...
        private void flpChicharron_MouseMove(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            ...
            if (!_capturandoDrag && Math.Abs(delta) > DragThreshold)
            {
                ...
                flpChicharron.Capture = true;   // capturar recién aquí
            }
            ...
            int currentY = -flpChicharron.AutoScrollPosition.Y;
            flpChicharron.AutoScrollPosition = new Point(target, currentY);

            _lastDX = x - _lastX;
            _lastX = x;
        }

        private void flpChicharron_MouseUp(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            ...
            if (_capturandoDrag)
            {
                flpChicharron.Capture = false; // liberar captura
                _velocity = _lastDX;
        ...
        {
            int currentX = -flpChicharron.AutoScrollPosition.X;
            int currentY = -flpChicharron.AutoScrollPosition.Y;
        ...
        {
            int overflow = flpChicharron.DisplayRectangle.Width - flpChicharron.ClientSize.Width;
            return Math.Max(0, overflow);
        }

        private int MouseXEnFlp()
        {
            return flpChicharron.PointToClient(Cursor.Position).X;
        }
    }
}
