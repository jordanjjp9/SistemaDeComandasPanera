...
{
    public partial class frmSandwich : Form, ISelectorProducto
    {
        ...
        public frmSandwich()
        {
            ...
            this.Load += frmSandwich_Load;

            flpSandwich.VerticalScroll.Visible = false;
            ...
            flpSandwich.MouseDown += flpSandwich_MouseDown;
            flpSandwich.MouseMove += flpSandwich_MouseMove;
            flpSandwich.MouseUp += flpSandwich_MouseUp;
        }

        private void frmSandwich_Load(object sender, EventArgs e)
        {
            ...
            flpSandwich.AutoScroll = true; // NECESARIO para que calcule el área de scroll

            // (Opcional) suaviza el repintado
            typeof(Panel).GetProperty("DoubleBuffered",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
                ?.SetValue(flpSandwich, true);

            // 🔹 Ocultar barras de scroll, pero mantener AutoScroll funcionando
            flpSandwich.HorizontalScroll.Enabled = false;
            flpSandwich.HorizontalScroll.Visible = false;
            flpSandwich.VerticalScroll.Enabled = false;
            flpSandwich.VerticalScroll.Visible = false;

            // (Si ya tienes botones en el diseñador)
            foreach (Control c in flpSandwich.Controls)
                WireChild(c);

            flpSandwich.ControlAdded += (_, ev) => WireChild(ev.Control);

            flpSandwich.AutoScrollPosition = new Point(1, 1);
        }
        ...
        {
            // Drag en los hijos
            c.MouseDown -= flpSandwich_MouseDown;
            c.MouseMove -= flpSandwich_MouseMove;
            c.MouseUp -= flpSandwich_MouseUp;
            c.MouseDown += flpSandwich_MouseDown;
            c.MouseMove += flpSandwich_MouseMove;
            c.MouseUp += flpSandwich_MouseUp;
        ...
        private void Child_EnterRedirectFocus(object sender, EventArgs e) => flpSandwich.Focus();
        private void Child_MouseDownRedirectFocus(object sender, MouseEventArgs e) => flpSandwich.Focus();

        private void flpSandwich_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            _originScrollX = -flpSandwich.AutoScrollPosition.X;
            _lastX = _dragStartX;
            _lastDX = 0;
            _velocity = 0;
            _inertiaTimer?.Stop();
        ...
        private void flpSandwich_MouseMove(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            ...
            int currentY = -flpSandwich.AutoScrollPosition.Y;
            flpSandwich.AutoScrollPosition = new Point(target, currentY);

            _lastDX = x - _lastX;
            _lastX = x;
        }

        private void flpSandwich_MouseUp(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            _dragging = false;
        ...
        {
            int currentX = -flpSandwich.AutoScrollPosition.X;
            int currentY = -flpSandwich.AutoScrollPosition.Y;
        ...
        {
            int overflow = flpSandwich.DisplayRectangle.Width - flpSandwich.ClientSize.Width;
            return Math.Max(0, overflow);
        }

        private int MouseXEnFlp() => flpSandwich.PointToClient(Cursor.Position).X;
