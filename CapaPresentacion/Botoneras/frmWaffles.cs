...
{
    public partial class frmWaffles : Form, ISelectorProducto
    {
        ...
        public frmWaffles()
        {
            ...
            this.Load += frmWaffles_Load;

            flpWaffles.VerticalScroll.Visible = false;
            ...
            flpWaffles.MouseDown += flpWaffles_MouseDown;
            flpWaffles.MouseMove += flpWaffles_MouseMove;
            flpWaffles.MouseUp += flpWaffles_MouseUp;
        }
        ...
        {
            // Drag en los hijos
            c.MouseDown -= flpWaffles_MouseDown;
            c.MouseMove -= flpWaffles_MouseMove;
            c.MouseUp -= flpWaffles_MouseUp;
            c.MouseDown += flpWaffles_MouseDown;
            c.MouseMove += flpWaffles_MouseMove;
            c.MouseUp += flpWaffles_MouseUp;
        ...
        private void Child_EnterRedirectFocus(object sender, EventArgs e) => flpWaffles.Focus();
        private void Child_MouseDownRedirectFocus(object sender, MouseEventArgs e) => flpWaffles.Focus();

        private void flpWaffles_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            _originScrollX = -flpWaffles.AutoScrollPosition.X;
            _lastX = _dragStartX;
            _lastDX = 0;
            _velocity = 0;
            _inertiaTimer?.Stop();
        ...
        private void flpWaffles_MouseMove(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            ...
            int currentY = -flpWaffles.AutoScrollPosition.Y;
            flpWaffles.AutoScrollPosition = new Point(target, currentY);

            _lastDX = x - _lastX;
            _lastX = x;
        }

        private void flpWaffles_MouseUp(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            _dragging = false;
        ...
        private void frmWaffles_Load(object sender, EventArgs e)
        {
            ...
            flpWaffles.AutoScroll = true; // NECESARIO para que calcule el área de scroll

            // (Opcional) suaviza el repintado
            typeof(Panel).GetProperty("DoubleBuffered",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
                ?.SetValue(flpWaffles, true);

            // 🔹 Ocultar barras de scroll, pero mantener AutoScroll funcionando
            flpWaffles.HorizontalScroll.Enabled = false;
            flpWaffles.HorizontalScroll.Visible = false;
            flpWaffles.VerticalScroll.Enabled = false;
            flpWaffles.VerticalScroll.Visible = false;

            // (Si ya tienes botones en el diseñador)
            foreach (Control c in flpWaffles.Controls)
                WireChild(c);

            flpWaffles.ControlAdded += (_, ev) => WireChild(ev.Control);

            flpWaffles.AutoScrollPosition = new Point(1, 1);
        }

        private void _inertiaTimer_Tick(object sender, EventArgs e)
        {
            int currentX = -flpWaffles.AutoScrollPosition.X;
            int currentY = -flpWaffles.AutoScrollPosition.Y;
        ...
        {
            int overflow = flpWaffles.DisplayRectangle.Width - flpWaffles.ClientSize.Width;
            return Math.Max(0, overflow);
        }


        private int MouseXEnFlp() => flpWaffles.PointToClient(Cursor.Position).X;
