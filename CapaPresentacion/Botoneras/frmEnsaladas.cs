...
{
    public partial class frmEnsaladas : Form, ISelectorProducto
    {
        ...
        public frmEnsaladas()
        {
            ...
            this.Load += frmEnsaladas_Load;

            flpEnsaladas.VerticalScroll.Visible = false;
            ...
            flpEnsaladas.MouseDown += flpEnsaladas_MouseDown;
            flpEnsaladas.MouseMove += flpEnsaladas_MouseMove;
            flpEnsaladas.MouseUp += flpEnsaladas_MouseUp;
        }

        private void frmEnsaladas_Load(object sender, EventArgs e)
        {
            ...
            flpEnsaladas.AutoScroll = true; // NECESARIO para que calcule el área de scroll

            // (Opcional) suaviza el repintado
            typeof(Panel).GetProperty("DoubleBuffered",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
                ?.SetValue(flpEnsaladas, true);

            // 🔹 Ocultar barras de scroll, pero mantener AutoScroll funcionando
            flpEnsaladas.HorizontalScroll.Enabled = false;
            flpEnsaladas.HorizontalScroll.Visible = false;
            flpEnsaladas.VerticalScroll.Enabled = false;
            flpEnsaladas.VerticalScroll.Visible = false;

            // (Si ya tienes botones en el diseñador)
            foreach (Control c in flpEnsaladas.Controls)
            {
                if (c is Button b)
                {
                    b.MouseDown += flpEnsaladas_MouseDown;
                    b.MouseMove += flpEnsaladas_MouseMove;
                    b.MouseUp += flpEnsaladas_MouseUp;
                }
            }

            flpEnsaladas.AutoScrollPosition = new Point(1, 1);
        }
        ...
        private void flpEnsaladas_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            _originScrollX = -flpEnsaladas.AutoScrollPosition.X;
            _lastX = _dragStartX;
            _lastDX = 0;
            _velocity = 0;
            _inertiaTimer?.Stop();
        ...
        private void flpEnsaladas_MouseMove(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            ...
            if (!_capturandoDrag && Math.Abs(delta) > DragThreshold)
            {
                ...
                flpEnsaladas.Capture = true;   // capturar recién aquí
            }
            ...
            int currentY = -flpEnsaladas.AutoScrollPosition.Y;
            flpEnsaladas.AutoScrollPosition = new Point(target, currentY);

            _lastDX = x - _lastX;
            _lastX = x;
        }

        private void flpEnsaladas_MouseUp(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            ...
            if (_capturandoDrag)
            {
                flpEnsaladas.Capture = false; // liberar captura
                _velocity = _lastDX;
        ...
        {
            int currentX = -flpEnsaladas.AutoScrollPosition.X;
            int currentY = -flpEnsaladas.AutoScrollPosition.Y;
        ...
        {
            int overflow = flpEnsaladas.DisplayRectangle.Width - flpEnsaladas.ClientSize.Width;
            return Math.Max(0, overflow);
        }

        private int MouseXEnFlp()
        {
            return flpEnsaladas.PointToClient(Cursor.Position).X;
        }
    }
}
