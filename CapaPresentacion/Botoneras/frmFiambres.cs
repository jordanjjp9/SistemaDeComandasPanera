...
{
    public partial class frmFiambres : Form, ISelectorProducto
    {
        ...
        public frmFiambres()
        {
            ...
            this.Load += frmFiambres_Load;

            flpFiambres.VerticalScroll.Visible = false;
            ...
            flpFiambres.MouseDown += flpFiambres_MouseDown;
            flpFiambres.MouseMove += flpFiambres_MouseMove;
            flpFiambres.MouseUp += flpFiambres_MouseUp;
        }

        private void frmFiambres_Load(object sender, EventArgs e)
        {
            ...
            flpFiambres.AutoScroll = true; // NECESARIO para que calcule el área de scroll

            // (Opcional) suaviza el repintado
            typeof(Panel).GetProperty("DoubleBuffered",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
                ?.SetValue(flpFiambres, true);

            // 🔹 Ocultar barras de scroll, pero mantener AutoScroll funcionando
            flpFiambres.HorizontalScroll.Enabled = false;
            flpFiambres.HorizontalScroll.Visible = false;
            flpFiambres.VerticalScroll.Enabled = false;
            flpFiambres.VerticalScroll.Visible = false;

            // (Si ya tienes botones en el diseñador)
            foreach (Control c in flpFiambres.Controls)
            {
                if (c is Button b)
                {
                    b.MouseDown += flpFiambres_MouseDown;
                    b.MouseMove += flpFiambres_MouseMove;
                    b.MouseUp += flpFiambres_MouseUp;
                }
            }

            flpFiambres.AutoScrollPosition = new Point(1, 1);
        }
        ...
        private void flpFiambres_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            _originScrollX = -flpFiambres.AutoScrollPosition.X;
            _lastX = _dragStartX;
            _lastDX = 0;
            _velocity = 0;
            _inertiaTimer?.Stop();
        ...
        private void flpFiambres_MouseMove(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            ...
            if (!_capturandoDrag && Math.Abs(delta) > DragThreshold)
            {
                ...
                flpFiambres.Capture = true;   // capturar recién aquí
            }
            ...
            int currentY = -flpFiambres.AutoScrollPosition.Y;
            flpFiambres.AutoScrollPosition = new Point(target, currentY);

            _lastDX = x - _lastX;
            _lastX = x;
        }

        private void flpFiambres_MouseUp(object sender, MouseEventArgs e)
        {
            if (!_dragging) return;
            ...
            if (_capturandoDrag)
            {
                flpFiambres.Capture = false; // liberar captura
                _velocity = _lastDX;
        ...
        {
            int currentX = -flpFiambres.AutoScrollPosition.X;
            int currentY = -flpFiambres.AutoScrollPosition.Y;
        ...
        {
            int overflow = flpFiambres.DisplayRectangle.Width - flpFiambres.ClientSize.Width;
            return Math.Max(0, overflow);
        }

        private int MouseXEnFlp()
        {
            return flpFiambres.PointToClient(Cursor.Position).X;
        }
    }
}
